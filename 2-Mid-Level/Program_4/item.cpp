...
// 2 add functions, 2 match functions, a compare function,
// and a display function. The private variables are char arrays of a name,
// a descriptions, a type, a platform, and an int rating out of 10
/*
   class item
   char* name;
   char* desc;	
...
int item::match(char* name_tofind)
{
	 if (!name_tofind)
		  return 0;
	 if (strcasecmp(name, name_tofind) == 0)
		  return 1;
...
int item::type_match(char* type_tofind)
{
	 if (!type_tofind)
		  return 0;
	 if (strcasecmp(type, type_tofind) == 0)
		  return 1;
...
int item::compare_name(char* name_tocomp)
{
	 return strcasecmp(name_tocomp, name);
}
